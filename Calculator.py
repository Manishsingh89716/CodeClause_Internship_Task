#Task2:- Create calculator

#import important modules
#Tkinter, provide the environment for gui window
import tkinter as tk
#messagebox, display the message box
import tkinter.messagebox
#sunken creates the design
from tkinter.constants import SUNKEN

#creating calculator window
window = tk.Tk()
window.title('Claculator')
frame = tk.Frame(master=window, bg="purple", padx=10)
frame.pack()
entry = tk.Entry(master=frame, relief=SUNKEN, borderwidth=3, width=30)
entry.grid(row=0, column=0, columnspan=3, ipady=2, pady=2)

#passing the number onclick
def myclick(number):
...
#clear the results and free the previous output
def clear():
    entry.delete(0, tk.END)

#create the buttons for numbers or operators which gives the result on clicking it.
#Buttons for the numbers upto 0 to 9
#lambda can take any number of arguments but have only one expression
